...
```{r}
ggplot(as.data.frame(table(factor(df$qRatingWorkshopMorningNPS, levels = 0:10))), aes(x=Var1, y=Freq)) + geom_bar(stat='identity', fill = 'darkred') + labs(x='score', y='count') + theme_bw()
```
...
```{r}
ggplot(as.data.frame(table(factor(df$qRatingWorkshopAfternoonNPS, levels = 0:10))), aes(x=Var1, y=Freq)) + geom_bar(stat='identity', fill = 'darkred') + labs(x='score', y='count') + theme_bw()
```
...
```{r}
ggplot(as.data.frame(table(factor(df$qContentRatingNPS, levels = 0:10))), aes(x=Var1, y=Freq)) + geom_bar(stat='identity', fill = 'darkred') + labs(x='score', y='count') + theme_bw()
```

### Keynote Rating
```{r}
ggplot(as.data.frame(table(factor(df$qKeynotesRatingNPS, levels = 0:10))), aes(x=Var1, y=Freq)) + geom_bar(stat='identity', fill = 'darkgreen') + labs(x='score', y='count') + theme_bw()
```
...
df_threechoice <- lapply(c('qAnnual_YesNo', 'qAttendNextYear_YesNo', 'qAttendIfFee_YesNo'), function(x) {
  df[, x] %>%
    rename('response' = !!x) %>%
    filter(!is.na(response)) %>%
...
```{r}
ggplot(as.data.frame(table(factor(df$qVenueNPS, levels = 0:10))), aes(x=Var1, y=Freq)) + geom_bar(stat='identity', fill = 'darkred') + labs(x='score', y='count') + theme_bw()
```
...
### Electronic Program
```{r}
ggplot(as.data.frame(table(factor(df$qProgramRatingNPS, levels = 0:10))), aes(x=Var1, y=Freq)) + geom_bar(stat='identity', fill = 'darkblue') + labs(x='score', y='count') + theme_bw()
```
